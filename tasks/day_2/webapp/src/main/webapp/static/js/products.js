...
const ADD_NEW_PRODUCT_TO_CART_SERVLET = ctx + '/order/add_product';
const REMOVE_PRODUCT_SERVLET = ctx + '/product-remove';
const GET_PRODUCT_COUNT_IN_CART = ctx + '/ajax/cart/product-count';
const PRODUCT_CART_ITEM_COUNTER = '#productCartItemCount';
const REMOVE_ALL_PRODUCT_OPTION ="#rmv-prod";


var CURRENT_CLICKED_BUY_BUTTON;

hangOnRemoveAllProducts();
...
function hangOnRemoveAllProducts(){
console.log("hanging on remove all");
     var element = document.querySelector(REMOVE_ALL_PRODUCT_OPTION);
     console.log(element);
     element.addEventListener('click',   (event) => {
        alert('You just in a moment from deleting all products. Are you sure?');
