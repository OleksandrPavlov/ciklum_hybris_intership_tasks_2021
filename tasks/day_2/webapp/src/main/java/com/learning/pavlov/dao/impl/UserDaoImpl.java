...
public class UserDaoImpl implements UserDao {
    ...
    public long add(User user) {
        ...
        try {
            return queryRunner.insert(connection, SQLQueriesConstants.INSERT_INTO_USERS, new ScalarHandler<BigInteger>(),
                    user.getLogin(),
                    user.getPassword()
            ).longValue();
    ...
    public Optional<User> findByLogin(String login) {
        Connection connection = ThreadLocalConnection.getConnection();
        try {
            return queryRunner.query(connection, SQLQueriesConstants.SELECT_USER_BY_LOGIN,
